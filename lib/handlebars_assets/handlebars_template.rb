...
module HandlebarsAssets
  ...
  class HandlebarsProcessor
    ...
    def call(input)
      ...
      if renderer.template_path.is_haml_6?
        string_buffer = engine.call(input[:data])
        html = eval(string_buffer)
        renderer.compile(html)
      else
  ...
  class HandlebarsRenderer
    ...
    def choose_engine(data)
      if @template_path.is_haml?
        if @template_path.is_haml_6?
          Haml::Engine.new(HandlebarsAssets::Config.haml_options)
        else
    ...
    class TemplatePath
      ...
      def is_haml_6?
        is_haml? && Gem::Version.new(Haml::VERSION) >= Gem::Version.new('6')
      end
